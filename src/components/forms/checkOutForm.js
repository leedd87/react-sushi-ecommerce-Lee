...
export const CheckOutForm = ({ finalPurchase, totalCompra, deleteCart }) => {
    ...
    const onHandleSubmit = (e) => {
        ...
        try {
            ...
            if (buyer.userEmail.match(mailFormat) && buyer.userName.match(nameFormat) && buyer.userPhone.match(phoneFormat)) {
                ...
                addDoc(orderInfo, newOrder).then(({ id }) => alert('Tu pedido esta en camino. El código es: ' + id));
    ...
    return (
        ...
        <form onSubmit={onHandleSubmit} className="row justify-content-center" name='formMensaje' id='formMensaje'>
            <div className="col-6 mb-2 row justify-content-center">
                ...
                <div>
                    <button type='submit' className='mt-5 col-7 col-lg-5 mb-1 fs-4'>Finalizar compra</button>
                </div>
            </div>
        </form>
    );
